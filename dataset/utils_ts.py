...
from dataset.datasets import EnergyDataset
...
def get_dataset(dataset_name: str, data_dir: str, logger: Any, configs: Any, **kwargs: Any) -> Any:
    """
    ...
    if os.path.exists(f"{path}.pkl"):
        ...
        if dataset_name == "london_energy":
            csv_path = f"{path}/london_energy.csv"
            df = pd.read_csv(csv_path)

            history_length = configs["data"]["history_length"]
            forward_length = configs["data"]["forward_length"]

            df["Date"] = pd.to_datetime(df["Date"])
            df = df.sort_values(["LCLid", "Date"])
            user_groups = df.groupby("LCLid")["KWH"].apply(lambda x: x.values.astype(np.float32)).tolist()
            user_groups = user_groups[:int(len(user_groups) * 0.1)]  # TODO 测试阶段仅仅使用前10%数据集

            split_idx = int(len(user_groups) * 0.75)
            train_series = user_groups[:split_idx]
            test_series = user_groups[split_idx:]

            all_data = EnergyDataset(train_series, history_length, forward_length)
            test_data = EnergyDataset(test_series, history_length, forward_length)
