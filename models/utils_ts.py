...
from models import AlexNet, CNN, MLP, WideResNet, LSTM
from trainers.default_trainer import train, inference, dp_train
# from trainers.ts_trainer import train, inference, dp_train
...
def get_model(model_type: str, dataset_name: str, configs: dict):
    ...
    if model_type == "RNN":
        ...
        input_size = configs["data"]["input_size"]
        output_size = configs["data"]["forward_length"]
...
def split_dataset_for_training(dataset_size, num_model_pairs):
    ...
    """
    # TODO 这一部分如果是时序数据集的话不能直接用indices区分成员非成员，除非在len(dataset)这一层已经是数据集中时间序列片段的数量。
    data_splits = []
