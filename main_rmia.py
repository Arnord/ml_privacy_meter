...
def collect_signal_lira(models_path,
    ...
    def load_or_compute_logits():
        ...
        primary_signal = "logits" + f"_augmented_{nb_augmentations}" * (
                    config["audit"]["augmentation"] == "augmented") + f"_num_ref_{nb_refs}" + last_folder
        ...
        model_logits = torch.from_numpy(create_or_load_signal_file(signal_path, dataset, num_queries=10,
                                                                   num_augmentations=nb_augmentations))  # TODO hard coded number of classes
        ...
        if np.isnan(model_logits[dataset_indeces, :]).sum() > 0:
            ...
            model_logits, _ = load_input_logits(models_path, epoch, model=model_index,
    ...
    def load_gradients():
        ...
        return load_input_gradient(models_path, epoch, model=model_index,
                                   num_augmentations=nb_augmentations), load_input_labels(models_path)

    if signal_name == "softmax_relative":
        extra = {}
        temperature = float(config['audit']['temperature'])
        primary_signal = "softmax" + f"_t_{temperature:.1f}" + f"_augmented_{nb_augmentations}" * (
                    config["audit"]["augmentation"] == "augmented") + f"_num_ref_{nb_refs}" + last_folder
        ...
        model_signal = torch.from_numpy(
            create_or_load_signal_file(signal_path, dataset, num_augmentations=nb_augmentations))
        ...
        if torch.isnan(model_signal[dataset_indeces]).sum() > 0:
            ...
            for k in range(nb_augmentations):
                model_signal[dataset_indeces, k] = convert_signals(model_logits[dataset_indeces, k, :], targets,
        ...
        primary_signal = f"softmax_margin_m_{config['audit']['taylor_m']}" + f"_t_{temperature:.1f}" + f"_augmented_{nb_augmentations}" * (
                    config["audit"]["augmentation"] == "augmented") + f"_num_ref_{nb_refs}" + last_folder
        ...
        if torch.isnan(model_signal[dataset_indeces]).sum() > 0:
            ...
            for k in range(nb_augmentations):
                model_signal[dataset_indeces, k] = convert_signals(model_logits[dataset_indeces, k, :], targets,
                                                                   "soft-margin", temp=temperature, extra=extra)
        ...
        temperature = float(config['audit']['temperature'])
        primary_signal = f"taylor_{config['audit']['taylor_n']}" + f"_t_{temperature:.1f}" + f"_augmented_{nb_augmentations}" * (
                    config["audit"]["augmentation"] == "augmented") + f"_num_ref_{nb_refs}" + last_folder
        ...
        if torch.isnan(model_signal[dataset_indeces]).sum() > 0:
            ...
            for k in range(nb_augmentations):
                model_signal[dataset_indeces, k] = convert_signals(model_logits[dataset_indeces, k, :], targets,
                                                                   "taylor", temp=temperature, extra=extra)
        ...
        temperature = float(config['audit']['temperature'])
        primary_signal = f"taylor_n_{config['audit']['taylor_n']}_m_{config['audit']['taylor_m']}" + f"_t_{temperature:.1f}" + f"_augmented_{nb_augmentations}" * (
                    config["audit"]["augmentation"] == "augmented") + f"_num_ref_{nb_refs}" + last_folder
        ...
        if torch.isnan(model_signal[dataset_indeces]).sum() > 0:
            ...
            for k in range(nb_augmentations):
                ...
                model_signal[dataset_indeces, k] = convert_signals(model_logits[dataset_indeces, k, :], targets,
                                                                   "taylor-soft-margin", temp=temperature, extra=extra)
    ...
    ## Note: To quickly implement another attack you can use another elif signal_name == "signal_name"
        ...
        primary_signal = "loss" + f"_augmented_{nb_augmentations}" * (
                    config["audit"]["augmentation"] == "augmented") + f"_num_ref_{nb_refs}" + last_folder
...
def aggregate_signals(signal_name,
    ...
    out_or_in_size = int(configs["audit"][
                             "num_ref_models"]) // 2  # min(configs["train"]["num_in_models"], configs["train"]["num_out_models"])
    ...
    if signal_name == "softmax_relative" or signal_name == "taylor_softmax_relative" or signal_name == "sm_taylor_softmax_relative" or signal_name == "sm_softmax_relative":  # relative attack
        ...
        if int(configs["audit"][
                   "top_k"]) != -1:  # selects for each x a random set of population equal to int(configs["audit"]["top_k"])
            top_k = int(configs["audit"]["top_k"])
            target_distances = kernel[target_indices][:,
                               population_indices]  # shape target_indices x population_indices
        ...
        if str2bool(configs["audit"]["offline"]):  # only taking out signals
            ...
            if float(configs["audit"]["offline"]) <= 1.0 and 0.0 <= float(configs["audit"]["offline"]):
                ...
                ref_signals = (torch.cat((in_signals[:, :in_number_of_models], out_signals[:, :out_number_of_models]),
                                         dim=1)).transpose(0, 1)

                print(ref_signals.shape)  # should be 1, 50000, 18

                print(
                    f"Fraction in models with {in_number_of_models} IN Models and {out_number_of_models} OUT models and {out_signals.shape[1]} models")
            else:
                raise Exception("ERROR : Online Proportion must be between 0.0 (offline) and 1.0 (online)")
        else:  # taking half in and half out
            ref_signals = (torch.cat((in_signals, out_signals), dim=1)).transpose(0,
                                                                                  1)  # shape nb_chosen_models x dataset_size x nb_augmentations
        ...
        if configs["audit"]["augmentation"] == "augmented":  # using augmentation
            ...
            if True:  # use all augmentations and population (by default)
                ...
                for k in tqdm(range(0, nb_augmentations),
                              desc=f"Relative attack for each query..."):  # computing attack scores for each augmentation
                    ...
                    if str2bool(configs["audit"]["offline"]):
                        if "ymedians" in configs["audit"].keys():
                            ...
                            prob_ratio_x = (target_signal[target_indices, k].ravel() / (
                                np.interp(mean_x, bin_centers, ymedian)))
                            prob_ratio_z_rev = 1 / (target_signal[population_indices, k].ravel() / (
                                np.interp(mean_z, bin_centers, ymedian)))
                        else:  # P(x) = (1+a)/2 P_OUT + (1-a)/2
                            prob_ratio_x = (target_signal[target_indices, k].ravel() / (
                                        (1 + offline_a) / 2 * mean_x + (1 - offline_a) / 2))
                            prob_ratio_z_rev = 1 / (target_signal[population_indices, k].ravel() / (
                                        (1 + offline_a) / 2 * mean_z + (1 - offline_a) / 2))
                    else:
                        prob_ratio_x = (target_signal[target_indices, k].ravel() / (mean_x))
                        prob_ratio_z_rev = 1 / (target_signal[population_indices, k].ravel() / (
                            mean_z))  # the inverse to compute the outer product
                ...
                prediction[:, population_indices] = ((prediction[:, population_indices] * len(mean_z)) - 1.0) / (
                            len(mean_z) - 1)  # to avoid overlapping cases
            ...
            if len(ref_signals.shape) == 3:
                ...
                if str2bool(configs["audit"]["offline"]):
                    if "ymedians" in configs["audit"].keys():
                        ...
                        prob_ratio_x = (target_signal[target_indices, 0].ravel() / (
                            np.interp(mean_x, bin_centers, ymedian)))
                        prob_ratio_z_rev = 1 / (target_signal[population_indices, 0].ravel() / (
                            np.interp(mean_z, bin_centers, ymedian)))
                    else:
                        prob_ratio_x = (target_signal[target_indices, 0].ravel() / (
                                    (1 + offline_a) / 2 * mean_x + (1 - offline_a) / 2))
                        prob_ratio_z_rev = 1 / (target_signal[population_indices, 0].ravel() / (
                                    (1 + offline_a) / 2 * mean_z + (
                                        1 - offline_a) / 2))  # the inverse to compute quickly
                else:
                    prob_ratio_x = (target_signal[target_indices, 0].ravel() / (mean_x))
                    prob_ratio_z_rev = 1 / (target_signal[population_indices, 0].ravel() / (
                        mean_z))  # the inverse to compute quickly
            else:
                ...
                if str2bool(configs["audit"]["offline"]):
                    if "ymedians" in configs["audit"].keys():
                        ...
                        prob_ratio_x = (
                                    target_signal[target_indices].ravel() / (np.interp(mean_x, bin_centers, ymedian)))
                        prob_ratio_z_rev = 1 / (target_signal[population_indices].ravel() / (
                            np.interp(mean_z, bin_centers, ymedian)))
                    else:
                        prob_ratio_x = (target_signal[target_indices].ravel() / (
                                    (1 + offline_a) / 2 * mean_x + (1 - offline_a) / 2))
                        prob_ratio_z_rev = 1 / (target_signal[population_indices].ravel() / (
                                    (1 + offline_a) / 2 * mean_z + (
                                        1 - offline_a) / 2))  # the inverse to compute quickly
                    ...
                    prob_ratio_z_rev = 1 / (
                                target_signal[population_indices].ravel() / (mean_z))  # the inverse to compute quickly
            ...
            np.savez(f"{log_dir}/{configs['audit']['report_log']}/p_x_{model_index}",  # to vizualize p_OUT vs. p_IN
                     ...
                     mean_x_in=in_signals[target_indices, :].mean(1),
                     # shape dataset_size x out_or_in_size x nb_augmentations
        ...
        if True:
            ...
            for data_idx in tqdm(range(nb_target)):
                identity_mask = (
                            all_population_indices != data_idx)  # check population that are NOT x, 1 could be False
                ...
                for population_idx in selected_population_idx:
                    ...
                    data_idx_signal.append(
                        (numerator_target + numerator_population - denominator_target - denominator_population).sum())
            ...
            signal_gamma = -((np.array(all_log_pdfs) > np.log(float(configs["audit"]["gamma"]))) * 1.0).mean(1).reshape(
                1, nb_target)
        ...
        if True:
            ...
            if os.path.exists(f"{log_dir}/{configs['audit']['report_log']}/fusion_{model_index}.npz"):
                ...
                numerator_x = torch.from_numpy(
                    data["numerator_x"][:, :, :nb_models])  # nb_target x population x nb_models
                denominator_x = torch.from_numpy(
                    data["denominator_x"][:, :, :nb_models])  # nb_target x population x nb_models

                numerator_z = torch.from_numpy(
                    data["numerator_z"][:, :, :nb_models])  # nb_target x population x nb_models
                denominator_z = torch.from_numpy(
                    data["denominator_z"][:, :, :nb_models])  # nb_target x population x nb_models
                ...
                for data_idx in tqdm(range(nb_target)):  # for each target point (warning : excrutiatingly slow)
                    ...
                    selected_population_idx = np.random.choice(all_population_indices[identity_mask], nb_pop,
                                                               replace=False)
                    ...
                    for population_idx in selected_population_idx:  # for each population sample
                        ...
                        intersection_in = reference_keep_matrix[:, data_idx] * (
                            ~reference_keep_matrix[:, population_idx])
                        intersection_out = (~reference_keep_matrix[:, data_idx]) * reference_keep_matrix[:,
                                                                                   population_idx]
                        ...
                        target_outs_x.append(
                            reference_signals[intersection_out, data_idx][:nb_models, 0]
                            # 0 because without augmentation.
                        )
                        ...
                        target_outs_z.append(
                            reference_signals[intersection_out, population_idx][:nb_models, 0]
                            # 0 because without augmentation.
                        )
                    ...
                    denominator_x.append(
                        target_outs_x)  # should be shape nb_target x population x (nb_reference / 4 in expectation)

                    numerator_z.append(target_ins_z)
                    denominator_z.append(
                        target_outs_z)  # should be shape nb_target x population x (nb_reference / 4 in expectation)
                ...
                target_scores_in_out = np.repeat(target_signal[np.arange(nb_target), 0].numpy(), nb_pop).reshape(-1,
                                                                                                                 1)  # MUST BE shape target * pop x 1

                # it's a column of pop_x_1[1], pop_x_1[2], pop_x_1[nb_pop] etc. then pop_x_2[1], pop_x_2[2], pop_x_2[nb_pop], etc.
                pop_scores_in_out = np.concatenate(
                    [target_signal[sel_pop_indices, 0].numpy().reshape(-1, 1) for sel_pop_indices in
                     per_target_selected_population], axis=0)  # MUST BE shape target * pop x 1
            ...
            if fix_variance:
                ...
                std_x_in = torch.std(input=unraveled_in_x, dim=1, keepdim=False).reshape(-1,
                                                                                         1).numpy()  # shape target * pop x 1
                std_x_out = torch.std(input=unraveled_out_x, dim=1, keepdim=False).reshape(-1, 1).numpy()

                std_z_in = torch.std(input=unraveled_in_z, dim=1, keepdim=False).reshape(-1,
                                                                                         1).numpy()  # shape target * pop x 1
    ...
    ## Note: To quickly implement another attack you can use another elif signal_name == "signal_name"
        ...
        if configs["audit"]["augmentation"] == "augmented":
            ...
            if fix_variance:
                if "allout" in configs["audit"].keys() and str2bool(
                        configs["audit"]["allout"]):  # reference points are all outside target's training set
        ...
        dat_reference_or_distill = np.sort(np.concatenate((losses, dummy_max, dummy_min), axis=0),
                                           axis=0)  # shape nb_models + 2 x nb_target
        ...
        ref = pop_target_signals.reshape(-1, 1).repeat_interleave(len(sc[0]),
                                                                  axis=1)  # to parallelize the computation of cdf
...
if __name__ == "__main__":
    ...
    if privacy_game == "privacy_loss_model":
        ...
        if target_path != reference_path:
            ref_logits, keep_matrix_lower = load_input_logits(reference_path, epoch,
                                                              num_augmentations=2)  # we want all_logits to be from the target_dir for consistent subset selection
        ...
        if "subset" in configs[
            "audit"].keys():  # typicality is computed from how high the correct logits are over multiple reference models
            ...
            for idx in range(nb_models_target):  # we consider the target_dir to be consistent across all experiments
                model_softmax = convert_signals(all_logits[idx, :, 0, :], targets, 'softmax', temp=1.0,
                                                extra=None)  # size 50k, get the softmax
        ...
        for model_index in list_target_models:
            ...
            if True:
                ...
                if configs["audit"]["signal"] in ["softmax_relative", "taylor_softmax_relative",
                                                  "sm_taylor_softmax_relative",
                                                  "sm_softmax_relative"]:  # relative attacks (hardcoded)
                    ...
                    prediction, answers = aggregate_signals(
                        ...
                        reference_signals,
                        (membership == False),
                        # Note : it can be changed to np.full(membership.shape, True) with top_k=25000 and results should be very close. No knowledge of OUT members is required in order for the attack to work.
                        configs,
                        kernel=pdistances
                    )
                    ...
                    plt.grid(True)
                    plt.legend()
                    plt.savefig(f"{log_dir}/{configs['audit']['report_log']}/score_distribution_{model_index}.png",
                                bbox_inches='tight')
                    plt.clf()
            ...
            correctly_labelled = ((-prediction).ravel() >= thresholds_dict[0.01]) == (
                answers.ravel())  # matrix of correctly labelled samples for FPR=1%
        ...
        logger.info(
            "TPR@1%%FPR of %.4f, TPR@0.1%%FPR of %.4f, TPR@0.01%%FPR of %.4f, TPR@0.001%%FPR of %.4f, TPR@0%%FPR of %.4f" % (
                np.mean(all_onep), np.mean(all_tenth), np.mean(all_hundredth), np.mean(all_thousandths),
                np.mean(all_zeros))
        )
